...
static void handle_motion_event(EmuEnvState &emuenv, int32_t sensor_type, const SensorEvent &sensor) {
    ...
    const auto rotate_sensor_data = [&](Util::Vec3f &data) {
    ...
    Util::Vec3f sensor_data = { sensor.data[0], sensor.data[1], sensor.data[2] };

    if (sensor_type == SDL_SENSOR_ACCEL) {
        sensor_data /= -SDL_STANDARD_GRAVITY;
        rotate_sensor_data(sensor_data);
...
void refresh_motion(MotionState &state, CtrlState &ctrl_state) {
    ...
    state.motion_data.UpdateOrientation(timestamp - state.last_refresh_motion_timestamp);
    state.motion_data.UpdateRotation(timestamp - state.last_refresh_motion_timestamp);

    state.last_refresh_motion_timestamp = timestamp;
