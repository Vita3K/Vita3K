...
std::vector<uint8_t> decrypt_fself(const std::vector<uint8_t> fself, const uint8_t *klic) {
    ...
    if (seg_infos->encryption != 2) {
        ...
        ElfHeader ehdr = ElfHeader(elf.data());
        ...
        hdr.self_filesize = HEADER_LEN + elf.size();
        ...
        for (int i = 0; i < ehdr.e_phnum; ++i) {
            ElfPhdr phdr = ElfPhdr(&elf[ehdr.e_phoff + (i * ehdr.e_phentsize)]);
            if (phdr.p_align > 0x1000)
                phdr.p_align = 0x1000;
            memcpy(&decrypted_self[hdr.phdr_offset + (i * ElfPhdr::Size)], &phdr, ElfPhdr::Size);
        }
        ...
        for (int i = 0; i < ehdr.e_phnum; ++i) {
            ...
            memcpy(&decrypted_self[hdr.section_info_offset + (i * SegmentInfo::Size)], &segment_info, SegmentInfo::Size);
        }

        // Copy the version info to the decrypted self
    ...
    // Self is not encrypted, return the original self
    return fself;
}
