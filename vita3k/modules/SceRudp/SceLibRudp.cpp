...
static void flush_buffer(RudpContext *ctx, std::vector<uint8_t> &buffer) {
    ...
    if (!buffer.empty()) {
        LOG_DEBUG_IF(log_rudp, "Flushed segments to send buffer, size: {}", buffer.size());
            ...
            if (!ctx->send_buffer.empty()) {
...

                {
                    ...
                    while ((offset + sizeof(RudpSegmentHeader)) <= res) {
                        ...
                        LOG_INFO_IF(log_rudp, "Success Sent: {} bytes - type: {}, seq: {}, index: {}, count: {}, size: {}, expected_size: {}",
...
static void rudp_io_read_thread(EmuEnvState &emuenv) {
    while (rudp.inited) {
        ...
        // LOG_INFO_IF(log_rudp, "[sceRudpEnableInternalIOThread] RUDP I/O thread woke up.");
        for (auto &[ctx_id, ctx] : rudp.contexts) {
            if (!ctx || (ctx->status.state == SCE_RUDP_STATE_IDLE))
                continue;
            ...
            if (!ctx->is_handled_by_poll) {
                ...
                rudp_poll_select({ { ctx_id, SCE_RUDP_POLL_EV_READ | SCE_RUDP_POLL_EV_WRITE, sock } }, static_cast<SceRudpUsec>(ctx->options[SCE_RUDP_OPTION_READ_TIMEOUT] * 1000), events_type);
            }
            ...
            if (res < 0) {
                LOG_ERROR("recv error: net err: {}, rudp err: {}", log_hex(res), log_hex(translate_return_value(res)));
                continue;
            }
            ...
            while ((offset + sizeof(RudpSegmentHeader)) <= res) {
                ...
                LOG_INFO_IF(log_rudp, "Received fragment: type={}, seq={}, index={}, count={}, size={}, expected_size={}",
            ...
            for (const auto &segment : segments) {
                ...
                case RUDP_SEGMENT_TYPE_SYN:
                    if ((ctx->status.state == SCE_RUDP_STATE_CLOSED) || (ctx->status.state == SCE_RUDP_STATE_SYN_SENT)) {
                        ...
                        LOG_INFO_IF(log_rudp, "Enqueued SYNCACK response (seq: {})", synack_segment.header.sequence_number);
                ...
                case RUDP_SEGMENT_TYPE_SYNCACK:
                    if ((ctx->status.state == SCE_RUDP_STATE_SYN_SENT) || (ctx->status.state == SCE_RUDP_STATE_SYN_RCVD)) {
                        ...
                        LOG_INFO_IF(log_rudp, "Connection successfully established.");
                        ...
                        LOG_INFO_IF(log_rudp, "Enqueued ACK response (seq: {})", ack_segment.header.sequence_number);
                ...
                case RUDP_SEGMENT_TYPE_DATA:
                    ...
                    if (segment.header.count > 1) {
                        ...
                        if (buffer.segments.size() == buffer.segment_count) {
                            ...
                            LOG_INFO_IF(log_rudp, "Reassembled and pushed full message (seq: {}, size: {})",
                ...
                case RUDP_SEGMENT_TYPE_ACK:
                    ...
                    case SCE_RUDP_STATE_SYN_RCVD:
                        ...
                        LOG_INFO_IF(log_rudp, "Connection successfully established (after ACK).");
                        break;
                    default:
...
EXPORT(int, sceRudpActivate, int ctx_id, const SceNetSockaddr *to, SceNetSocklen_t to_len) {
    ...
    LOG_INFO_IF(log_rudp, "sceRudpActivate(ctx_id={}, to_len={})", ctx_id, to_len);
...
EXPORT(int, sceRudpBind, int ctx_id, int soc, uint16_t vport, uint8_t mux_mode) {
    ...
    LOG_INFO_IF(log_rudp, "sceRudpBind(ctx_id={}, soc={}, vport={}, mux_mode={})", ctx_id, soc, ntohs(vport), mux_mode);
    if (!rudp.inited)
        return RET_ERROR(SCE_RUDP_ERROR_NOT_INITIALIZED);
...
EXPORT(int, sceRudpGetContextStatus, int ctx_id, SceRudpContextStatus *status, size_t status_size) {
    ...
    LOG_INFO_IF(log_rudp, "sceRudpGetContextStatus, ctx id: {}", ctx_id);
    ...
    *status = ctx->status;
    ...
    LOG_INFO_IF(log_rudp, "SceRudpGetContextStatus: ctx_id={}, state={}, lostPackets={}, sentPackets={}, rcvdPackets={}, sentBytes={}, rcvdBytes={}, retransmissions={}, rtt={}",
...
EXPORT(SceSSize, sceRudpGetSizeReadable, int ctx_id) {
    ...
    LOG_INFO_IF(log_rudp, "sceRudpGetSizeReadable(ctx_id={})", ctx_id);
...
EXPORT(SceSSize, sceRudpGetSizeWritable, int ctx_id) {
    ...
    LOG_INFO_IF(log_rudp, "sceRudpGetSizeWritable(ctx_id={})", ctx_id);
...
EXPORT(int, sceRudpGetStatus, SceRudpStatus *status, uint32_t status_size) {
    ...
    LOG_INFO_IF(log_rudp, "sceRudpGetStatus(status_size={})", status_size);
    if (!rudp.inited)
        return RET_ERROR(SCE_RUDP_ERROR_NOT_INITIALIZED);
...
EXPORT(int, sceRudpInitiate, int ctx_id, const struct SceNetSockaddr *to, SceNetSocklen_t to_len, uint16_t vport) {
    ...
    LOG_INFO_IF(log_rudp, "sceRudpInitiate, ctx id: {}, to len: {}, vport: {}", ctx_id, to_len, vport);
...
EXPORT(int, sceRudpPollControl, int pollId, int op, int ctxId, uint16_t events) {
    ...
    LOG_INFO_IF(log_rudp, "sceRudpPollControl: pollId={}, op={}, ctxId={}, events={}", pollId, rudp_poll_op_to_string(op), ctxId, rudp_poll_event_flags_to_string(events));
...
EXPORT(int, sceRudpPollCreate, uint32_t size) {
    ...
    LOG_INFO_IF(log_rudp, "sceRudpPollCreate: id={}, size={}", poll_id, size);
    return poll_id;
}
...
EXPORT(int, sceRudpPollWait, int poll_id, SceRudpPollEvent *events, uint32_t event_len, SceRudpUsec timeout) {
    ...
    LOG_INFO_IF(log_rudp, "sceRudpPollWait: poll_id={}, event_len={}, timeout={}", poll_id, event_len, timeout);
...
EXPORT(int, sceRudpRead, int ctx_id, void *data, uint32_t len, uint8_t flags, SceRudpReadInfo *info) {
    ...
    LOG_INFO_IF(log_rudp, "sceRudpRead(ctx_id={}, data={}, len={}, flags={:#x})", ctx_id, (void *)data, len, flags);
    if (!rudp.inited)
        return RET_ERROR(SCE_RUDP_ERROR_NOT_INITIALIZED);
...
EXPORT(int, sceRudpSetMaxSegmentSize, uint16_t mss) {
    ...
    LOG_INFO_IF(log_rudp, "SceRudpSetMaxSegmentSize: mss={}", mss);
    if (!rudp.inited)
        return RET_ERROR(SCE_RUDP_ERROR_NOT_INITIALIZED);
...
EXPORT(int, sceRudpWrite, int ctx_id, const void *data, uint32_t len, uint8_t flags) {
    ...
    LOG_INFO_IF(log_rudp, "sceRudpWrite(ctx_id={}, data={}, len={}, flags={:#x})", ctx_id, (void *)data, len, flags);
