...
static int SDLCALL thread_function(EmuEnvState &emuenv) {
    ...
    bool holdEventPowerTriggered = false;
    bool pressEventPowerTriggered = false;
    bool pressEventTriggered = false;
    std::chrono::steady_clock::time_point pressStartTime;
    std::chrono::steady_clock::time_point pressStartPowerTime;
    int current_event = SCE_POWER_CB_BUTTON_PS_POWER_PRESS;
    while (true) {
        int num_keys;
        ...
        if (currentPowerState && !previousPowerState) {
            pressStartPowerTime = std::chrono::steady_clock::now();
            holdEventPowerTriggered = false;
            pressEventPowerTriggered = false;
        } else if (currentPowerState) {
            std::chrono::steady_clock::time_point currentTime = std::chrono::steady_clock::now();
            std::chrono::duration<double> pressDuration = currentTime - pressStartPowerTime;
        ...
        if (currentKeyState && !previousKeyState) {
            ...
            std::chrono::steady_clock::time_point currentTime = std::chrono::steady_clock::now();
            std::chrono::duration<double> pressDuration = currentTime - pressStartTime;
