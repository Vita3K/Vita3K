...
EXPORT(int, sceNpTusCreateTitleCtx, const np::CommunicationID *communicationId,
    ...
    if (communicationId)
        LOG_DEBUG("sceNpTusCreateTitleCtx(communicationId: {}, num: {})", communicationId->data, communicationId->num);

    if (passphrase)
        LOG_DEBUG("sceNpTusCreateTitleCtx, passphrase: {}", reinterpret_cast<const char *>(passphrase->data));
