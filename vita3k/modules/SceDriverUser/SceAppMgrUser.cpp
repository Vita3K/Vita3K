...
EXPORT(int, sceAppMgrGetAppParam, char *param) {
    ...
    const std::string param_str = fmt::format("type=LAUNCH_APP_BY_URI&uri=psgm:play?titleid={}", emuenv.io.title_id);
    std::strncpy(param, param_str.c_str(), param_str.size());
...
EXPORT(int, sceAppMgrLaunchAppByName, int flags, const char *name, const char *param) {
    ...
    const auto complet_path = fmt::format("vs0:app/{}/eboot.bin", name);
    /*const auto app_module_uid = load_module(emuenv, complet_path);
    if (app_module_uid < 0) {
        LOG_ERROR("Fail to load app: {}", complet_path);
        return -1;
    }
    const auto module = emuenv.kernel.loaded_modules[app_module_uid];
    LOG_INFO("App executable {} ({}) loaded", module->info.module_name, complet_path);

    if (param && param[0] != '\0') {
        size_t len = strlen(param) + 1;
        std::vector<uint8_t> buf(param, param + len);
        auto arr = Ptr<uint8_t>(alloc(emuenv.mem, static_cast<uint32_t>(buf.size()), "app_param"));
        memcpy(arr.get(emuenv.mem), buf.data(), buf.size());
        Ptr<void> param_ptr(arr.address());
        if (start_module(emuenv, module->info, len - 1, param_ptr) < 0) {
            LOG_ERROR("Failed to run app thread.");
            return -1;
        }
    }*/
