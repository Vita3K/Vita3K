...
static void check_empty_param(EmuEnvState &emuenv, const SceAppUtilSaveDataSlotEmptyParam *empty_param, const uint32_t idx) {
    ...
    if (empty_param) {
        ...
        if (iconPath) {
            ...
            vfs::read_file(VitaIoDevice::ux0, emuenv.common_dialog.savedata.icon_buffer[idx], emuenv.pref_path, thumbnail_path);
        } else if (iconBuf && (iconBufSize > 0)) {
            emuenv.common_dialog.savedata.icon_buffer[idx].insert(emuenv.common_dialog.savedata.icon_buffer[idx].end(), iconBuf, iconBuf + iconBufSize);
        }
    }
}
