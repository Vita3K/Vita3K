...
static std::map<std::string, std::shared_ptr<DownloadTask>> download_tasks;
static bool notice_info_state;

void draw_download_notifications(GuiState &gui, EmuEnvState &emuenv) {
    ...
    DownloadNotification n; 
    {
        std::lock_guard<std::mutex> lock(notice_mutex);
        auto &org = gui.download_notifications.back();
        if (!org.active) {
            org.active = true;
            org.start_time = now;
            alpha = 0.0f;
    ...
    if (notice_info_state && !fading_out) {
        fading_out = true;
    }

    constexpr float DISPLAY_TIME = 6.f;
    ...
    if (fading_out) {
        if (fade_towards(alpha, 0.0f, fade_out_speed)) {
            std::lock_guard<std::mutex> lock(notice_mutex);
            gui.download_notifications.pop_back();
            return;
        }
    }
    ...
    const auto &app_icon = gui.app_selector.vita_apps_icon[fmt::format("ux0:app/{}", n.app_id)];
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(GUI_COLOR_TEXT_BLACK.x, GUI_COLOR_TEXT_BLACK.y, GUI_COLOR_TEXT_BLACK.z, alpha));
...
static void download_update(GuiState &gui, EmuEnvState &emuenv, const std::string &id, const std::string &group) {
    DownloadTask *task = nullptr;
    {
        ...
        auto it = download_tasks.find(group);
        if (it == download_tasks.end()) {
    ...
    bool success = net_utils::download_file(task->url, update_path_str, progress_callback);

    const auto is_download_paused = !success && !dp->state.download && dp->is_paused.load();
    const auto is_download_cancelled = !success && !dp->state.download && !dp->is_paused.load();
    bool is_download_completed = success && dp->state.download;
    bool is_download_failed = !success && !is_download_paused && !is_download_cancelled;
    ...
    if (!is_download_completed && !is_download_failed)
        return; // Download was cancelled or paused, no further action needed

    {
        std::lock_guard<std::mutex> lock(notice_mutex);
        const auto result = is_download_completed ? "wait_install" : "download_failed";
        update_notice_info(gui, emuenv, result, id);
        if (!notice_info_state) {
            const auto APP_INDEX = get_app_index(gui, fmt::format("ux0:app/{}", id));
            const auto title = APP_INDEX ? APP_INDEX->title : id;
            gui.download_notifications.insert(gui.download_notifications.begin(),
                { id, title, is_download_completed ? gui.lang.indicator["downloading_complete"] : emuenv.common_dialog.lang.common["download_failed"] });
        }
    }
...
static void destroy_download_threads() {
    for (auto &[group, task] : download_tasks) {
...
static void download_manager(GuiState &gui, EmuEnvState &emuenv) {
    while (download_manager_running.load()) {
        ...
        download_cv.wait(lock, [] {
            return !download_manager_running.load() || !download_tasks.empty();
        });
        lock.unlock();
...

        {
            ...
            for (auto it = download_tasks.begin(); it != download_tasks.end();) {
                ...
                // Check if the download thread has finished, join if necessary
                const bool thread_finished = !dp->state.download && dt->thread.joinable();
                if (thread_finished) {
                    LOG_DEBUG("Joining finished download thread for group: {}", it->first);
                    dt->thread.join();
                }

                // Remove completed downloads that are neither paused nor waiting
                const bool should_remove = !dp->is_paused.load() && !dp->is_waiting.load() && !dp->state.download;
                if (should_remove) {
                    LOG_DEBUG("Removing completed download task for group: {}", it->first);
        ...
        const auto has_active_download = std::any_of(download_tasks.begin(), download_tasks.end(), [](const auto &pair) {
        ...
        if (has_active_download)
            ...
            std::lock_guard<std::mutex> lock(download_manager_mutex);
            auto it = std::find_if(download_tasks.begin(), download_tasks.end(),
                [](const auto &pair) {
                    auto &dp = pair.second->download_progress;
            ...
            if (it != download_tasks.end()) {
                auto &dt = it->second;
                auto &dp = dt->download_progress;
                dp->state.download = true;
                dp->is_waiting = false;
                dt->thread = std::thread(download_update, std::ref(gui), std::ref(emuenv), dt->id, it->first);
...
static bool set_notice_info(GuiState &gui, EmuEnvState &emuenv, const NoticeList &info) {
    ...
    if (info.type == "content") {
        ...
        // Load remote info if not already done
        if (!gui.new_update_infos.contains(info.id))
            get_remote_update_info(gui, emuenv, info.id);
        ...
        task->id = info.id;
        task->path = emuenv.pref_path / "ux0/bgdl/t" / info.group / fs::path(update_infos.url).filename();
        ...
        download_tasks.emplace(info.group, task);
...
void init_notice_info(GuiState &gui, EmuEnvState &emuenv) {
    ...
    if (!download_manager_thread.joinable()) {
        download_manager_thread = std::thread(download_manager, std::ref(gui), std::ref(emuenv));
    ...
    for (const auto &task : notice_tasks) {
        ...
        case NoticeAction::REMOVE_FULL: {
            ...
            if (notice_it != notice_list[task.user_id].end()) {
                ...
                LOG_INFO("Notice content: user: {}, id: {}, content_id: {}, group: {}, type: {}, time: {} has been removed.", task.user_id, notice_it->id, notice_it->content_id, notice_it->group, notice_it->type, notice_it->time);
...
static void draw_notice_info(GuiState &gui, EmuEnvState &emuenv) {
    ...
    const ImVec2 NOTICE_SIZE = notice_info_count_new ? ImVec2(110.0f * SCALE.x, 100.0f * SCALE.y) : ImVec2(90.0f * SCALE.x, 82.0f * SCALE.y);
    ...
    const ImVec2 WINDOW_SIZE = notice_info_state ? VIEWPORT_SIZE : (notice_info_count_new ? ImVec2(88.f * SCALE.x, 78.f * SCALE.y) : ImVec2(72.f * SCALE.x, 72.f * SCALE.y));
    ...
    const auto CLIP_SIZE = notice_info_count_new ? 24.f : 19.f;
    draw_list->PushClipRect(ImVec2(NOTICE_ICON_POS.x + (CLIP_SIZE * SCALE.x), NOTICE_ICON_POS.y),
        ImVec2(NOTICE_ICON_POS_MAX.x, NOTICE_ICON_POS_MAX.y - (CLIP_SIZE * SCALE.y)), true);
    ...
    if (notice_info_state) {
        ...
        if (notice_info.empty()) {
            ...
            ImGui::SetColumnWidth(0, 88.f * SCALE.x);
            ...
            for (const auto &notice : notice_info) {
                ...
                ImGui::SetCursorPosY(ICON_POS.y + begin_title_space);
                ...
                if (notice.type.find("downloading") != std::string::npos) {
                    ...
                    if (dt) {
                        ...
                        ImGui::SetCursorPosY(ICON_POS.y + (45.f * SCALE.y));
                        ImGui::PushStyleColor(ImGuiCol_PlotHistogram, GUI_PROGRESS_BAR);
                        ImGui::ProgressBar(dp->progress.load(), ImVec2(602.f * SCALE.x, 12.f * SCALE.y), "");
                        ImGui::PopStyleColor();
                        ImGui::SetCursorPosY(ICON_POS.y + (60.f * SCALE.y));
                ...
                if ((notice.type.find("download") != std::string::npos) || (notice.type == "wait_install")) {
                    ...
                    ImGui::PopStyleColor(2);
                    ImGui::PopStyleVar();
                    const ImVec2 BUTTON_SIZE = ImVec2(200.f * SCALE.x, 60.f * SCALE.y);
                    ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 16.f * SCALE.x);
                    ...
                    const auto POPUP_HEIGHT = BUTTON_SIZE.y * BUTTON_COUNT + ((BUTTON_COUNT - 1) * 1.f);
                    ImGui::SetNextWindowSize(ImVec2(BUTTON_SIZE.x, POPUP_HEIGHT), ImGuiCond_Always);
                    ImGui::SetNextWindowPos(ImVec2(btn_pos.x - BUTTON_SIZE.x - (45.f * SCALE.x), btn_pos.y + (button_size.y / 2.f) + (6.f * SCALE.y)), ImGuiCond_Always);
                    if (ImGui::BeginPopupModal(fmt::format("download_menu_{}", notice.group).c_str(), nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings)) {
                        ...
                        if (notice.type == "download_failed") {
                            if (ImGui::Button(lang["resume"].c_str(), BUTTON_SIZE)) {
                                notice_tasks.push_back({ NoticeAction::REMOVE_FULL, notice.time, "global" });
                            ...
                            if (dt) {
                                ...
                                if (ImGui::Button(button_str.c_str(), BUTTON_SIZE)) {
                        ...
                        draw_list->AddRect(p0, p1, BORDER_TRANSLUCIDE, 16.f * SCALE.x, 0, BORDER_SIZE);

                        const ImVec2 tip_top = ImVec2(p1.x + HALF_BORDER_SIZE, p0.y + (28.f * SCALE.y) - HALF_BORDER_SIZE);
                        const ImVec2 tip_right = ImVec2(p1.x + HALF_BORDER_SIZE + (20.f * SCALE.x), p0.y + (20.f * SCALE.y) - HALF_BORDER_SIZE);
                        const ImVec2 tip_bottom = ImVec2(p1.x + HALF_BORDER_SIZE, p0.y + (44.f * SCALE.y) - HALF_BORDER_SIZE);
                        draw_list->AddTriangleFilled(tip_top, tip_right, tip_bottom, BORDER_TRANSLUCIDE);
                ...
                ImGui::SetCursorPos(ImVec2(INDICATOR_SIZE.x - (34.f * SCALE.x) - notice_time_size.x, ICON_POS.y + (66.f * SCALE.y)));
        ...
        if (!notice_info.empty()) {
            ...
            const auto MENU_BUTTON_SIZE = ImVec2(246.f * SCALE.x, 60.f * SCALE.y);
            ...
            const auto button_size = ImVec2(64.f * SCALE.x, 40.f * SCALE.y);
            ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, button_size.x);
            ...
            if (ImGui::BeginPopupModal("ellipsis_menu", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings)) {
                ...
                auto set_download_state = [&](bool pause) {
                    {
                        ...
                        for (auto &[group, dt] : download_tasks) {
                            auto &dp = dt->download_progress;
                ...
                if (ImGui::BeginPopupModal("Delete All", nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoSavedSettings)) {
                    ...
                    if (ImGui::Button(common["ok"].c_str(), BUTTON_SIZE) || ImGui::IsKeyPressed(static_cast<ImGuiKey>(emuenv.cfg.keyboard_button_cross))) {
                        for (const auto &info : notice_info) {
                            if ((info.type.find("downloading") == std::string::npos) && (info.type != "wait_install")) {
                                const auto user_id = info.type == "trophy" ? emuenv.io.user_id : "global";
                                notice_tasks.push_back({ NoticeAction::REMOVE_FULL, info.time, user_id });
                            }
                        }
                        if (notice_info.empty())
                            notice_info_state = false;
                    ...
                    ImVec2 p0 = ImVec2(ImGui::GetWindowPos().x - HALF_BORDER_SIZE, ImGui::GetWindowPos().y - HALF_BORDER_SIZE);
                    ImVec2 p1 = ImVec2(p0.x + ImGui::GetWindowSize().x + BORDER_SIZE, p0.y + ImGui::GetWindowSize().y + BORDER_SIZE);
                    draw_list->AddRect(p0, p1, border_color, 16.f * SCALE.x, 0, BORDER_SIZE);
                    
                    const ImVec2 tip_top = ImVec2(p1.x + HALF_BORDER_SIZE, p1.y - (28.f * SCALE.y) - HALF_BORDER_SIZE); 
                    const ImVec2 tip_right = ImVec2(p1.x + HALF_BORDER_SIZE + (20.f * SCALE.x), p1.y - (20.f * SCALE.y) - HALF_BORDER_SIZE);
                    const ImVec2 tip_bottom = ImVec2(p1.x + HALF_BORDER_SIZE, p1.y - (44.f * SCALE.y) - HALF_BORDER_SIZE);
                    draw_list->AddTriangleFilled(tip_top, tip_right, tip_bottom, border_color);
                }
