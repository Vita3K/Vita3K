...
static bool init_notice_icon(GuiState &gui, EmuEnvState &emuenv, const fs::path &content_path, const NoticeList &info) {
    gui.notice_info_icon[info.time].clear();
    int32_t width = 0;
    int32_t height = 0;
...
static bool set_notice_info(GuiState &gui, EmuEnvState &emuenv, const NoticeList &info) {
    ...
    notice_tasks.push_back({ NoticeAction::ADD, info.time });
    ...
    if (type == "content") {
        ...
        if (emuenv.app_info.app_category == "gp") {
            ...
            if (NOTICE_INDEX != notice_list["global"].end()) {
                ...
                notice_tasks.push_back({ NoticeAction::REMOVE_ICON, NOTICE_INDEX->time });
        ...
        notice_tasks.push_back({ NoticeAction::REMOVE_ICON, info.time });
        ...
        if (it != notice_list["global"].end())
            notice_tasks.push_back({ NoticeAction::REMOVE_ICON, it->time });
...
static void process_notice_tasks(EmuEnvState &emuenv, GuiState &gui) {
    for (const auto &task : notice_tasks) {
        ...
        case NoticeAction::REMOVE_FULL: {
            ...
            LOG_DEBUG("Removing notice with time: {}", task.time);

            const auto notice_it = std::find_if(notice_list["global"].begin(), notice_list["global"].end(), is_notice_deleted);
            if (notice_it != notice_list["global"].end()) {
                if (notice_it->is_new) {
                    --notice_list_count_new["global"];
                    --notice_info_count_new;
                }
                ...
                notice_list["global"].erase(notice_it);
            }
        ...
        case NoticeAction::ADD: {
            auto it = std::find_if(notice_list["global"].begin(), notice_list["global"].end(), [&](const auto &n) {
            ...
            if (!set_notice_icon(gui, emuenv, *it)) {
                LOG_ERROR("Failed to set notice icon for time: {}", task.time);
                // If setting notice info fails, remove it from the list
                notice_list["global"].erase(it);
                save_notice_list(emuenv);
                continue;
            }
