...
#ifdef _WIN32
        static const char OS_PREFIX[] = "start \"RPCSV\" ";
#elif __APPLE__
...
void draw_app_context_menu(GuiState &gui, EmuEnvState &emuenv, const std::string &app_path) {
    ...
    const auto EXPORT_TEXTURES_PATH{ emuenv.shared_path / "textures/export" / title_id };
    const auto IMPORT_TEXTURES_PATH{ emuenv.shared_path / "textures/import" / title_id };
    const auto ISSUES_URL = "https://github.com/RPCSV/compatibility/issues";
            ...
            if (ImGui::BeginMenu(lang_compat.name.c_str())) {
                if (!is_commercial_app || !gui.compat.compat_db_loaded) {
                    if (ImGui::MenuItem(lang.main["check_app_state"].c_str())) {
                        const std::string compat_url = is_commercial_app ? "https://RPCSV.org/compatibility?g=" + title_id : "https://github.com/RPCSV/homebrew-compatibility/issues?q=" + APP_INDEX->title;
                    ...
                    ImGui::Spacing();
                    if (has_state_report) {
                        const auto copy_RPCSV_summary = [&]() {
                            const auto RPCSV_summary = fmt::format(
                                "# RPCSV summary\n- Version: {}\n- Build number: {}\n- Commit hash: https://github.com/RPCSV/RPCSV/commit/{}\n- GPU backend: {}",
                                app_version, app_number, app_hash, emuenv.cfg.backend_renderer);
                            ImGui::LogToClipboard();
                            ImGui::LogText("%s", RPCSV_summary.c_str());
                            ImGui::LogFinish();
                        };
                        if (ImGui::MenuItem(lang.main["copy_RPCSV_summary"].c_str()))
                            copy_RPCSV_summary();
                        ...
                        if (ImGui::MenuItem(lang.main["create_state_report"].c_str())) {
                            ...
                            // Create RPCSV summary
                            const auto RPCSV_summary = fmt::format(
                                "%23 RPCSV summary%0A- Version: {}%0A- Build number: {}%0A- Commit hash: https://github.com/RPCSV/RPCSV/commit/{}%0A- GPU backend: {}",
                                app_version, app_number, app_hash, emuenv.cfg.backend_renderer);
                            ...
                            const auto rest_of_body = "%23 Issues%0A<!-- Summary of problems -->%0A%0A%23 Screenshots%0A![image](https://?)%0A%0A%23 Log%0A%0A%23 Recommended labels%0A<!-- See https://github.com/RPCSV/compatibility/labels -->%0A- A?%0A- B?%0A- C?";

                            open_path(fmt::format(
                                "{}/new?assignees=&labels=&projects=&template=1-ISSUE_TEMPLATE.md&title={} [{}]&body={}%0A%0A{}%0A%0A{}%0A%0A{}",
                                ISSUES_URL, title, title_id, app_summary, RPCSV_summary, test_env_summary, rest_of_body));
                        }
                    }
                }
