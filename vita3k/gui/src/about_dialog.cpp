...
void draw_about_dialog(GuiState &gui, EmuEnvState &emuenv) {
    ...
    ImGui::Text("%s", lang["RPCSV"].c_str());
    ImGui::Spacing();
    ImGui::TextWrapped("%s", lang["about_RPCSV"].c_str());
    ...
    ImGui::SameLine(0, 0);
    open_link("GitHub", "https://github.com/RPCSV/RPCSV");
    ImGui::Spacing();

    ImGui::Text("%s ", lang["RPCSV_website"].c_str());
    ImGui::SameLine(0, 0);
    open_link("RPCSV.org", "https://RPCSV.org");
    ImGui::Spacing();

    ImGui::Text("%s ", lang["ko-fi_website"].c_str());
    ImGui::SameLine(0, 0);
    open_link("Ko-fi", "https://ko-fi.com/RPCSV");

    ImGui::Spacing();
    ImGui::Separator();
    ImGui::Spacing();

    // Draw RPCSV Staff list
    TextColoredCentered(GUI_COLOR_TEXT_MENUBAR, lang["RPCSV_staff"].c_str());
    ImGui::Spacing();
    ...
    if (ImGui::BeginTable("##RPCSV_staff_table", STAFF_COLUMN_COUNT, ImGuiTableFlags_ScrollY | ImGuiTableFlags_BordersV | ImGuiTableFlags_NoSavedSettings, STAFF_LIST_SIZE)) {
        ...
        const auto open_author_history = [&](const char *author) {
            open_link(author, fmt::format("https://github.com/RPCSV/RPCSV/commits?author={}", author).c_str());
        };
