...
bool init_RPCSV_update(GuiState &gui) {
    // Reset all variables
    state = NO_UPDATE;
    ...
    const auto version = net_utils::get_web_regex_result("https://api.github.com/repos/RPCSV/RPCSV/releases/latest", std::regex("RPCSV Build: (\\d+)"));
    ...
    if (version.empty() || !std::all_of(version.begin(), version.end(), ::isdigit)) {
        ...
        gui.help_menu.RPCSV_update = false;
        return false;
    }
        ...
        std::thread get_commit_desc([dif_from_current]() {
            ...
            const auto get_commits = [dif_from_current]() {
                ...
                for (auto p = 0; p < page_count; p++) {
                    ...
                    const auto continuous_link = fmt::format(R"(https://api.github.com/repos/RPCSV/RPCSV/commits?sha=continuous&page={}&per_page={})", p + 1, std::min(dif_from_current, max_per_page));
    ...
    if (has_update || gui.help_menu.RPCSV_update) {
...
static void download_update(const fs::path &base_path) {
    ...
    std::thread download([base_path]() {
        std::string download_continuous_link = "https://github.com/RPCSV/RPCSV/releases/download/continuous";
#ifdef _WIN32
        ...
        const auto RPCSV_latest_ver_path = base_path / "RPCSV-latest-ver";
        const auto RPCSV_latest_path = base_path / ("RPCSV-latest" + archive_ext);

        const std::string RPCSV_version = std::to_string(git_version);

        // check if RPCSV_latest_path exist
        if (fs::exists(RPCSV_latest_path)) {
            // read latest ver file if exist
            if (fs::ifstream RPCSV_latest_ver{ RPCSV_latest_ver_path, std::ios::binary }) {
                std::string RPCSV_latest_version;
                std::getline(RPCSV_latest_ver, RPCSV_latest_version);

                // check if latest version of RPCSV is same with current git version for can resume download
                if (RPCSV_latest_version == RPCSV_version)
                    LOG_INFO("Resume download of version: {}", RPCSV_latest_version);
                else {
                    fs::remove(RPCSV_latest_ver_path);
                    fs::remove(RPCSV_latest_path);
                    LOG_INFO("Start download of new version: {}", RPCSV_version);
                }
            }
        }

        // check if latest_ver file exist
        if (!fs::exists(RPCSV_latest_ver_path)) {
            // write latest_info file
            if (fs::ofstream RPCSV_latest_ver{ RPCSV_latest_ver_path, std::ios::binary }) {
                RPCSV_latest_ver.write(RPCSV_version.c_str(), RPCSV_version.length());
            } else {
                LOG_ERROR("Failed to write latest_ver file at {}", RPCSV_latest_ver_path.string());
            }
        }

        // Download latest RPCSV version
        LOG_INFO("Attempting to download and extract the latest RPCSV version {} in progress...", git_version);
        ...
        if (net_utils::download_file(download_continuous_link, fs_utils::path_to_utf8(RPCSV_latest_path), progress_callback)) {
            SDL_Event event;
            event.type = SDL_EVENT_QUIT;
            SDL_PushEvent(&event);

#ifdef _WIN32
            const auto RPCSV_batch = fmt::format("\"{}\\update-RPCSV.bat\"", base_path);
            FreeConsole();
#elif defined(__APPLE__)
            const auto RPCSV_batch = fmt::format("sh \"{}/update-RPCSV.sh\"", base_path);
#else
            const auto RPCSV_batch = fmt::format("chmod +x \"{}/update-RPCSV.sh\" && \"{}/update-RPCSV.sh\"", base_path, base_path);
#endif

            // When success finish download, remove latest ver file
            fs::remove(RPCSV_latest_ver_path);

            std::system(RPCSV_batch.c_str());
        } else {
...
static std::string get_remaining_str(LangState &lang, const uint64_t remaining) {
    if (remaining > 60)
        return fmt::format(fmt::runtime(lang.RPCSV_update["minutes_left"]), remaining / 60);
    else
        return fmt::format(fmt::runtime(lang.RPCSV_update["seconds_left"]), remaining > 0 ? remaining : 1);
}

void draw_RPCSV_update(GuiState &gui, EmuEnvState &emuenv) {
    ...
    auto &lang = gui.lang.RPCSV_update;

    ImGui::SetNextWindowPos(WINDOW_POS, ImGuiCond_Always);
    ImGui::SetNextWindowSize(display_size, ImGuiCond_Always);
    ImGui::Begin("##RPCSV_update", &gui.help_menu.RPCSV_update, ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoSavedSettings);
    ...
    case DESCRIPTION: {
        ...
        for (const auto &desc : git_commit_desc_list) {
            ...
            if (ImGui::Selectable("##commit_link", false, ImGuiSelectableFlags_SpanAllColumns, ImVec2(ImGui::GetWindowWidth(), max_height))) {
                open_path(fmt::format("https://github.com/RPCSV/RPCSV/commit/{}", desc.sha));
            }
            ImGui::PopID();
            ImGui::Separator();
        }
    ...
    case UPDATE_RPCSV:
        ImGui::SetCursorPosY((display_size.y / 2.f) - ImGui::GetFontSize());
        TextCentered(lang["update_RPCSV"].c_str());
    ...
    if (state > NO_UPDATE && state < DOWNLOAD) {
        ...
        if (ImGui::Button(state < UPDATE_RPCSV ? lang["next"].c_str() : lang["update"].c_str(), BUTTON_SIZE) || ImGui::IsKeyPressed(static_cast<ImGuiKey>(emuenv.cfg.keyboard_button_circle))) {
            state = (RPCSVUpdate)(state + 1);
            if (state == DOWNLOAD)
