    ...
    case EDIT: {
        ...
        if (ImGui::Button(lang["choose_avatar"].c_str(), CHANGE_AVATAR_BTN_SIZE)) {
            ...
            if (result == host::dialog::filesystem::Result::SUCCESS) {
                ...
                fs::create_directories(temp_avatar_path.parent_path());
                FILE *infile = host::dialog::filesystem::resolve_host_handle(avatar_path);
                if (!infile) {
                    LOG_ERROR("Failed to load pkg file in path: {}", avatar_path.generic_path().string());
                    avatar_path.clear();
                } else {
                    FILE *outfile = fopen(temp_avatar_path.string().c_str(), "wb");
                    if (!outfile) {
                        LOG_ERROR("Failed to create temp avatar file in path: {}", temp_avatar_path.generic_path().string());
                        fclose(infile);
                        avatar_path.clear();
                    } else {
...
#endif
                if (!avatar_path.empty() && init_avatar(gui, emuenv, "temp", avatar_path.string()))
