...
void init_vita_apps(GuiState &gui, EmuEnvState &emuenv) {
    ...
    const auto old_apps_list_size = gui.app_selector.vita_apps["ux0"].size();
    gui.app_selector.vita_apps.clear();

    init_fw_apps(gui, emuenv);

    std::thread init_apps([&, old_apps_list_size]() {
        ...
        if (old_apps_list_size == new_apps_list_size)
            return false;

        int diff = std::abs((int)new_apps_list_size - (int)old_apps_list_size);
        std::string change_app_list = (new_apps_list_size > old_apps_list_size) ? "new application(s) added" : "application(s) removed";
            
        LOG_INFO("{} {}", diff, change_app_list);
...
void draw_home_screen(GuiState &gui, EmuEnvState &emuenv) {
    ...
    const auto display_app = [&](const std::vector<gui::App> &apps_list) {
        ...
        for (const auto &app : apps_list) {
            ...
            apps_list_filtered.push_back({app.path, current_line, current_col});
