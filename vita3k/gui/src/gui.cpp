        ...
        for (int i = 0; i < font_scale_count; i++) {
            ...
            if (fs::exists(latin_fw_font_path)) {
                ...
                if (fs::exists(default_font_path)) {
                    ...
                    LOG_INFO("Using default RPCSV font.");
                } else
                    LOG_WARN("Could not find default RPCSV font at {}, using default ImGui font.", default_font_path);
            }
        }
...
void draw_vita_area(GuiState &gui, EmuEnvState &emuenv) {
    ...
    if (gui.help_menu.RPCSV_update)
        draw_RPCSV_update(gui, emuenv);
