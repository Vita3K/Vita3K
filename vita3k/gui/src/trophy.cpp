...
bool get_trophy_np_com_id_list(GuiState &gui, HostState &host) {
    ...
    if (!fs::exists(trophy_path) || fs::is_empty(trophy_path)) {
        LOG_WARN("Trophy path is empty");
        return false;
    }
    ...
    for (const auto &trophy : fs::directory_iterator(trophy_conf_path)) {
        if (!trophy.path().empty() && fs::is_directory(trophy.path())) {
            ...
            // Read Updated time
            const auto trophy_data_np_com_id_path{ fs::path(trophy_path) / "data" / np_com_id / "TROPUSR.DAT" };
            const auto updated = fs::last_write_time(trophy_data_np_com_id_path);
            ...
            if (!load_trophy_progress(host.io, progress_input_file, np_com_id)) {
                LOG_ERROR("Fail load trophy progress for title: {}", np_com_id);
                return false;
            }
...
static void get_trophy_list(GuiState &gui, HostState &host, const std::string &np_com_id, const std::string &group_id) {
        ...
        for (const auto &conf : doc.child("trophyconf")) {
            if (conf.name() == std::string("trophy")) {
                ...
                if (group_id == gid) {
                    ...
                    trophy_info[trophy_id].ttype = conf.attribute("ttype").as_string();
    ...
    for (const auto &trophy : trophy_info) {
        ...
        if (trophy_info[trophy_id].ttype == "P")
            trophy_info[trophy_id].type = "Platinium";
        else if (trophy_info[trophy_id].ttype == "G")
            trophy_info[trophy_id].type = "Gold";
        else if (trophy_info[trophy_id].ttype == "S")
            trophy_info[trophy_id].type = "Silver";
        else if (trophy_info[trophy_id].ttype == "B")
            trophy_info[trophy_id].type = "Bronze";
...
static std::string np_com_id_selected, group_id_selected, trophy_id_selected;
...
void draw_trophy_selection(GuiState &gui, HostState &host) {
    ...
    const auto SIZE_LIST = ImVec2(780 * SCAL.x, 452.f * SCAL.y);
    ...
    if (np_com_id_selected.empty()) {
        ...
        if (group_id_selected.empty()) {
            ...
            for (const auto &trophy : trophy_list) {
                ...
                ImGui::TextColored(GUI_COLOR_TEXT, "%s", hidden_trophy ? "?" : trophy_info[trophy.id].ttype.c_str());
                ImGui::NextColumn();
                ImGui::PushStyleVar(ImGuiStyleVar_SelectableTextAlign, ImVec2(0.f, 0.2f));
            ...
            ImGui::TextColored(GUI_COLOR_TEXT, "%s", hidden_trophy ? "?" : trophy_info[trophy_id_selected].type.c_str());
    ...
    if (ImGui::Button("Back", ImVec2(80.f * SCAL.x, 50.f * SCAL.y))) {
        if (!np_com_id_selected.empty()) {
            if (detail_np_com_id) {
                ...
                if (np_com_id_info[np_com_id_selected].group_id.size() == 1) {
                    group_id_selected.clear();
    ...
    if (np_com_id_selected.empty() || !group_id_selected.empty()) {
        ...
        if (ImGui::Button("...", ImVec2(60.f * SCAL.x, 50.f * SCAL.y)) || ImGui::IsKeyPressed(host.cfg.keyboard_button_triangle))
            ImGui::OpenPopup("...");
