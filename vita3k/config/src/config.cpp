...
ExitCode init_config(Config &cfg, int argc, char **argv, const Root &root_paths, bool portable) {
    ...
    fs::path config_path = root_paths.get_config_path();
    ...
    if (fs::exists(check_path(config_path))) {
        parse(cfg, config_path, root_paths.get_vita_fs_path());
    } else {
        serialize_config(command_line, check_path(config_path));
    }

    // Declare all options
    {
        ZoneScopedN("Parse command line");
        ...
        config->add_option("--backend-renderer,-B", command_line.backend_renderer, "Renderer backend to use")
            ->ignore_case()
            ->check(CLI::IsMember(std::set<std::string>{ "OpenGL", "Vulkan" }))
