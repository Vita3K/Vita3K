...
private:
    ...
    const SavedMemory *find_saved_by_other_cheat(uint32_t address, uint32_t size) const {
        for (const Cheat &other : m_state.file.cheats) {
            if (&other == &m_cheat)
                continue;
            for (const SavedMemory &saved : other.saved_memory) {
                if ((saved.address == address) && (saved.bytes.size() == size))
                    return &saved;
            }
        }
    ...
    bool execute_arm_write(const CodeLine &line) {
        ...
        if (!already_saved) {
            ...
            // Another cheat may have patched this address already, what it saved is the real original.
            if (const SavedMemory *original = find_saved_by_other_cheat(address, size)) {
    ...
    bool execute_relative_base(const CodeLine &line) {
        ...
        if ((module_index != 0) || m_state.modules.empty()) {
            LOG_WARN_ONCE("Cheat '{}' refers to module {}, only module 0 (the main executable) is supported", m_cheat.name, module_index);
            return false;
        }
        if (segment_index >= 2) {
            LOG_WARN_ONCE("Cheat '{}' refers to segment {}, which does not exist", m_cheat.name, segment_index);
            return false;
        }
