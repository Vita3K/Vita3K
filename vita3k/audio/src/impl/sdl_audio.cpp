...
void SDLCALL SDLAudioAdapter::thread_wakeup_callback(void *userdata, SDL_AudioStream *stream, int additional_amount, int total_amount) {
    ...
    if (port->thread >= 0) {
        ...
        // Running out of data?
        if (samples_available < static_cast<SDLAudioAdapter &>(port->adapter).device_buffer_samples + 2 * port->len) {
            // Wake the thread up.
...
void SDLAudioAdapter::audio_output(ThreadState &thread, AudioOutPort &out_port, const void *buffer) {
    ...
    // *2 to make sure one buffer can be ready
    // ((device_buffer_samples + port.len - 1) / port.len + 1) * port.len is more correct but too complex
    if (samples_available >= device_buffer_samples + 2 * port.len) {
