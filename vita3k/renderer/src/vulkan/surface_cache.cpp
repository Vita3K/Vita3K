...
static void protect_surface(MemState &mem, ColorSurfaceCacheInfo &info) {
    ...
    MemPerm perm = supports_surface_sync ? MemPerm::None : MemPerm::ReadOnly;
    MemPerm existing_perm;
    if (is_protecting(mem, addr_start, &existing_perm))
        perm = existing_perm;

    std::shared_ptr<bool> need_sync = supports_surface_sync ? info.need_surface_sync : nullptr;

    add_protect(mem, addr_start, addr_end - addr_start, perm,
        [dirty = info.dirty, need_sync](Address, bool write) {
            if (write)
                *dirty = true;
            else if (need_sync)
                *need_sync = true;
