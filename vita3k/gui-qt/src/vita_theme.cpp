...
fs::path resolve_asset_path(const fs::path &theme_dir, std::string asset_value) {
    asset_value = string_utils::trim_copy(std::move(asset_value));
    if (asset_value.empty())
        return {};

    std::replace(asset_value.begin(), asset_value.end(), '\\', '/');

    const auto drive_separator = asset_value.find(':');
    if (drive_separator != std::string::npos)
        asset_value = asset_value.substr(drive_separator + 1);

    while (asset_value.starts_with("./"))
        asset_value.erase(0, 2);
    while (!asset_value.empty() && asset_value.front() == '/')
        asset_value.erase(asset_value.begin());

    if (asset_value.empty())
        return {};

    return (theme_dir / fs_utils::utf8_to_path(asset_value)).lexically_normal();
}
...
std::string stylesheet_path_url(const fs::path &path) {
    std::string normalized = fs_utils::path_to_utf8(path);
    std::replace(normalized.begin(), normalized.end(), '\\', '/');
    return normalized;
}

struct ThemeTemplateContext {
    std::unordered_map<std::string, VitaThemeColor> colors;
...
std::optional<std::string> render_template_token(
    ...
    if (separator == std::string::npos) {
        const auto string_it = context.strings.find(token);
        if (string_it == context.strings.end())
            return std::nullopt;
        return string_it->second;
    }

    const std::string kind = token.substr(0, separator);
    const std::string rest = token.substr(separator + 1);

    if (kind == "str") {
        const auto it = context.strings.find(rest);
    ...
    if (kind == "int") {
        const auto it = context.numbers.find(rest);
        if (it == context.numbers.end())
            return std::nullopt;
        return std::to_string(it->second);
    }

    if (kind == "hex") {
        const auto it = context.colors.find(rest);
        if (it == context.colors.end())
            return std::nullopt;
        return css_hex(it->second);
    }

    if (kind == "rgba" || kind == "argb") {
        const auto color_separator = rest.find(':');
        if (color_separator == std::string::npos)
            return std::nullopt;

        const std::string color_key = rest.substr(0, color_separator);
        const std::string alpha_key = rest.substr(color_separator + 1);
        const auto color_it = context.colors.find(color_key);
        const auto alpha_it = context.numbers.find(alpha_key);
        if (color_it == context.colors.end() || alpha_it == context.numbers.end())
            return std::nullopt;

        const std::uint8_t alpha = static_cast<std::uint8_t>(std::clamp(alpha_it->second, 0, 255));
        return (kind == "rgba")
            ? css_rgba(color_it->second, alpha)
            : css_argb(color_it->second, alpha);
    }
...
std::string build_stylesheet(
    ...
    const std::string background_url = stylesheet_path_url(background_option.asset_path);
    ...
    put("template_version", std::string(gui::VitaThemeTemplateVersion));
    put("generated_theme_id", theme.theme_id);
    put("generated_background_id", background_option.id);
    put("generated_readability_percent", std::to_string(readability_percent));
    put("generated_normalize_font_colors", normalize_font_colors ? "true" : "false");
...
std::optional<VitaThemeInfo> load_vita_theme(
    ...
    theme.theme_xml_path = theme_xml_path;
    ...
    for (const auto &background_node : collect_background_nodes(home_node)) {
        ...
        option.preview_path = *asset_path;
    ...
    if (!theme.background_options.empty())
        theme.home_preview_path = theme.background_options.front().preview_path;

    if (theme.home_preview_path.empty()) {
        const std::string home_preview_raw = [&]() {
            if (const std::string preview = child_text(home_node, "m_previewFilePath"); !preview.empty())
                return preview;
            return child_text(home_node, "m_homePreviewFilePath");
        }();
        if (const auto preview_path = existing_asset_path(theme_dir, home_preview_raw))
            theme.home_preview_path = *preview_path;
    }
...
std::optional<VitaThemeSynthesisResult> synthesize_vita_theme_qss(
    const VitaThemeInfo &theme,
    const std::string &background_option_id,
    const int readability_percent,
    const bool normalize_font_colors,
    ...
    VitaThemeSynthesisResult result;
    result.theme_name = fmt::format("vita-theme-{}", theme.theme_id);
    result.selected_background_id = selected_background->id;
    result.output_directory = gui_settings_dir / "vita-themes" / theme.theme_id;
    result.qss_path = result.output_directory / "generated.qss";
    result.stylesheet = build_stylesheet(
        theme,
        *selected_background,
    ...
    boost::system::error_code error_code;
    fs::create_directories(result.output_directory, error_code);
    if (error_code) {
    ...
    fs::ofstream output(result.qss_path, fs::ofstream::binary | fs::ofstream::trunc);
    if (!output) {
        LOG_ERROR("Failed to open generated Vita theme stylesheet path {}", result.qss_path);
        return std::nullopt;
    }

    output.write(result.stylesheet.data(), static_cast<std::streamsize>(result.stylesheet.size()));
    if (!output.good()) {
        LOG_ERROR("Failed to write generated Vita theme stylesheet {}", result.qss_path);
        return std::nullopt;
    }
