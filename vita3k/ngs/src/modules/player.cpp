    ...
    if (static_cast<int>(state->decoded_samples_pending) < granularity) {
        ...
        while (static_cast<int>(state->decoded_samples_pending) < granularity) {
            ...
            if (data.extra_storage.size() < sizeof(float) * 2 * granularity
                ...
                // Workaround for certain Unity games with playback frequency <= 1.0 and a different sample rate
                if ((static_cast<int>(round(src_sample_rate)) != sample_rate) && (src_sample_rate <= 1.f)) {
                    LOG_INFO_ONCE("The current game uses a playback frequency <= 1.0 and a different sample rate. Setting playback frequency to match the sample rate: {}.", sample_rate);
                    src_sample_rate = sample_rate;
                }

                if ((params->playback_scalar != 1.f) || (static_cast<int>(round(src_sample_rate)) != sample_rate)) {
