...
static bool init_web_camera(Camera *self) {
    ...
    int num_cameras;
    std::unique_ptr<SDL_CameraID[], SDL_Deleter> cameras(SDL_GetCameras(&num_cameras));
    if (!cameras) {
...
static bool init_image(Camera *self) {
    ...
    if (self->pImpl->format != SDL_PIXELFORMAT_RGBA8888) {
...
void Camera::update_config(int type, const std::string &id, const std::string &image, int color) {
