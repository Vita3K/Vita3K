...
static bool read_file_from_zip(Buffer &eboot, FILE *&vpk_fp, const char * file, const ZipPtr zip) {
    if (!mz_zip_reader_init_cfile(zip.get(), vpk_fp, 0, 0)) {
        return false;
    }

    const int eboot_index = mz_zip_reader_locate_file(zip.get(), file, nullptr, 0);
    if (eboot_index < 0) {
        return false;
    }

    if (!mz_zip_reader_extract_file_to_callback(zip.get(), file, &write_to_buffer, &eboot, 0)) {
        return false;
    }
