...
EXPORT(int, sceRtcCheckValid, const SceDateTime *pTime) {
    if (pTime == nullptr) {
        return error(export_name, SCE_RTC_ERROR_INVALID_POINTER);
    }
    if (pTime->month < 1 || pTime->month > 12) {
        return error(export_name, SCE_RTC_ERROR_INVALID_MONTH);
    }
    if (pTime->day < 1) {
        return error(export_name, SCE_RTC_ERROR_INVALID_DAY);
    }
    switch (pTime->month) {
    case 4: // April
    case 6: // June
    ...
    if (pTime->hour > 23) {
        return error(export_name, SCE_RTC_ERROR_INVALID_HOUR);
    }
    if (pTime->minute > 59) {
        return error(export_name, SCE_RTC_ERROR_INVALID_MINUTE);
    }
    if (pTime->second > 59) {
        return error(export_name, SCE_RTC_ERROR_INVALID_SECOND);
    }
    if (pTime->microsecond > 99999) {
        return error(export_name, SCE_RTC_ERROR_INVALID_MICROSECOND);
    }
    return 0;
}
...
EXPORT(int, sceRtcGetDayOfWeek, int year, int month, int day) {
    ...
    if (year < 0) {
        return error(export_name, SCE_RTC_ERROR_INVALID_YEAR);
    }
    if (day < 1) {
        return error(export_name, SCE_RTC_ERROR_INVALID_DAY);
    }
