...
#define HOOK_FOREACH(uc, hh, idx)                                                                      \
    for (                                                                                              \
        ...
        cur != NULL && ((hh) = (struct hook *)cur->data) /* stop excuting callbacks on stop request */ \
        && !uc->stop_request;                                                                          \
        cur = cur->next)
...
struct uc_struct {
    ...
    QTAILQ_HEAD(memory_listeners, MemoryListener)
    memory_listeners;
    QTAILQ_HEAD(, AddressSpace)
    address_spaces;
