...
struct bf_t : bf_base<T, N> {
    ...
    struct extract_impl<T2, std::enable_if_t<std::is_unsigned<T2>::value>> {
        ...
        static constexpr T2 extract(const T &data) {
            ...
            return static_cast<T2>((static_cast<utype>(data) >> bitpos) & ((1 << bf_t::bitsize) - 1));
        }
    };
    ...
    // Bitfield extraction
    static constexpr type extract(const T &data) {
        return extract_impl<type>::extract(data);
    }

    // Bitfield insertion
    static constexpr type insert(type value) {
        return static_cast<type>((value & bf_t::vmask) << bitpos);
    }
    ...
    bf_t &operator=(type value) {
        this->m_data = static_cast<type>((this->m_data & ~data_mask()) | insert(value));
        return *this;
    }

    type operator++(int) {
        utype result = *this;
        *this = static_cast<type>(result + 1);
        return result;
    }
    ...
    bf_t &operator&=(type right) {
        this->m_data &= static_cast<type>((static_cast<utype>(right) & bf_t::vmask) << bitpos);
        return *this;
    }
...
struct cf_t : bf_base<typename F::type, F::bitsize + cf_t<Fields...>::bitsize> {
    ...
    static constexpr type data_mask() {
        return static_cast<type>(F::data_mask() | cf_t<Fields...>::data_mask());
    }

    // Extract all bitfields and concatenate
    static constexpr type extract(const type &data) {
        return static_cast<type>(static_cast<utype>(F::extract(data)) << cf_t<Fields...>::bitsize | cf_t<Fields...>::extract(data));
    }

    // Split bitfields and insert them
    static constexpr type insert(type value) {
        return static_cast<type>(F::insert(value >> cf_t<Fields...>::bitsize) | cf_t<Fields...>::insert(value));
    }
